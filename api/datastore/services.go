...
import (
	...
	"github.com/portainer/portainer/api/dataservices/serviceinstancescheduledbuild"
...
type Store struct {
	...
	ServiceInstanceScheduledBuildService *serviceinstancescheduledbuild.Service
...
func (store *Store) initServices() error {
	...
	serviceInstanceScheduledBuildService, err := serviceinstancescheduledbuild.NewService(store.connection)
	if err != nil {
		return err
	}
	store.ServiceInstanceScheduledBuildService = serviceInstanceScheduledBuildService
...
// ServiceInstanceScheduledBuild gives access to the ServiceInstanceScheduledBuild data management layer
func (store *Store) ServiceInstanceScheduledBuild() dataservices.ServiceInstanceScheduledBuildService {
	return store.ServiceInstanceScheduledBuildService
}
...
type storeExport struct {
	...
	ServiceInstanceScheduledBuild []portainer.ServiceInstanceScheduledBuild `json:"service_instance_scheduled_builds,omitempty"`
...
func (store *Store) Export(filename string) (err error) {
	...
	if sibs, err := store.ServiceInstanceScheduledBuild().ReadAll(); err != nil {
		if !store.IsErrObjectNotFound(err) {
			log.Error().Err(err).Msg("exporting Service Instance Scheduled Builds")
		}
	} else {
		backup.ServiceInstanceScheduledBuild = sibs
	}
...
func (store *Store) Import(filename string) (err error) {
	...
	for _, v := range backup.ServiceInstanceScheduledBuild {
		if err := store.ServiceInstanceScheduledBuild().Update(v.ID, &v); err != nil {
			log.Warn().Err(err).Msg("failed to update the service instance scheduled build in the database")
		}
	}
